...
strip = neopixel.NeoPixel(NEOPIXEL_PIN, NUM_PIXELS, brightness=0.2, auto_write=False)
chase = Chase(strip, speed=0.1, color=WHITE, size=10, spacing=5)
...
COLOR_IDLE = colorwheel(255)
COLOR_SWING = COLOR_IDLE
...
mixer = audiomixer.Mixer(voice_count=3, sample_rate=11025, channel_count=1, bits_per_sample=16, samples_signed=True)
...
while True:
    ...
    COLOR_IDLE = intColorC
    if not switch.value:                    # button pressed?
        if mode == 0:                       # If currently off...
            enable.value = True
            power('on2', 1.7, False)         # Power up!
            speaker.play(mixer)
